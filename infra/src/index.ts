...
import { runRecovery } from '../../recovery/recovery-orchestrator'
import { runHealthProbes } from '../../recovery/health/probes'
import type { HealthProbe } from '../../recovery/types'

type WorkerExecutionContext = { waitUntil: (promise: Promise<unknown>) => void }
type WorkerScheduledEvent = { cron?: string }
...
async function handleRequest(req: Request, env: Env, ctx: WorkerExecutionContext): Promise<Response> {
  ...
  const probes: HealthProbe[] = await runHealthProbes(env, { skipWorkerProbe: url.pathname === '/api/health' })
    const allOk = probes.every(p => p.ok)
    const active = await resolveBackend(env)
    const backend = active ? await checkBackend({ ...env, BACKEND_ORIGIN: active }) : 'down'
    const body = { ok: allOk, worker: 'up', backend, probes, ts: new Date().toISOString() }
    const res = json(body, { status: allOk ? 200 : 503, headers: cors })
  ...
  if (url.pathname === '/api/recovery/run') {
    if (req.method !== 'POST') {
      const res = json({ ok: false, error: 'Method Not Allowed' }, { status: 405, headers: cors })
      res.headers.set('x-request-id', reqId)
      return res
    }
    const outcome = await runRecovery(req, env)
    const res = json(outcome.body, { status: outcome.status, headers: cors })
    res.headers.set('x-request-id', reqId)
    return res
  }
...
export default {
  async fetch(req: Request, env: Env, ctx: WorkerExecutionContext) {
    try {
  ...
  async scheduled(_event: WorkerScheduledEvent, env: Env, ctx: WorkerExecutionContext) {
    const active = await resolveBackend(env)
