...
export function ClinicsConfiguration({ language }) {
  ...
  const [modifiedRoutes, setModifiedRoutes] = useState(() => cloneRoutes(EXAM_ROUTES))
  ...
  const handleCancelEdit = () => {
    if (hasChanges) {
      const ok = confirm(language === 'ar' ? 'لديك تغييرات غير محفوظة، هل تريد إلغاءها؟' : 'You have unsaved changes, discard them?')
      if (!ok) return
    }
    setModifiedRoutes(cloneRoutes(EXAM_ROUTES))
    setHasChanges(false)
    setEditMode(false)
  }
  ...
  const handleAddClinic = (examKey, floorKey) => {
    ...
    if (clinicName && clinicName.trim()) {
      const newRoutes = cloneRoutes(modifiedRoutes)
  ...
  const getFloorColor = (floorKey) => {
    switch (floorKey) {
      case 'mezzanine': return 'bg-blue-500/10 border-blue-400/50'
      case 'second': return 'bg-green-500/10 border-green-400/50'
      case 'third': return 'bg-purple-500/10 border-purple-400/50'
      default: return 'bg-slate-700/30 border-slate-500/40'
    }
  }
  ...
  return (
    <div className="space-y-6" dir="rtl">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-white">تكوين العيادات والمسارات</h1>
          ...
          <CardContent>
            <div className="space-y-6">
              {Object.entries(modifiedRoutes[selectedExam].floors).map(([floorKey, floor]) => (
                <div key={floorKey} className={`p-4 rounded-lg border-2 text-white ${getFloorColor(floorKey)}`}>
                  <div className="flex items-center gap-3 mb-4">
                    <div className="w-8 h-8 bg-slate-900/70 rounded-full flex items-center justify-center font-bold text-lg border border-slate-500/60 text-white">
                      {getFloorIcon(floorKey)}
                    </div>
                    <div>
                      <h3 className="font-semibold text-lg text-white">{floor.name}</h3>
                      {floor.note && (
                        <p className="text-sm text-gray-300 mt-1">{floor.note}</p>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                    {floor.clinics.map((clinic, index) => (
                      <div key={index} className="flex items-center justify-between gap-3 p-3 bg-gray-900/70 rounded-lg border border-gray-600">
                        <div className="flex items-center gap-3 flex-1 min-w-0">
                          <div className="text-blue-300 flex-shrink-0">
                            {getClinicIcon(clinic)}
                          </div>
                          <div className="flex-1 min-w-0">
                            <div className="font-medium text-sm truncate text-white">{clinic}</div>
                        ...
                        {editMode && (
                          <Button
                            ...
                            variant="outline"
                            size="sm"
                            className="text-red-400 border-red-400/50 hover:bg-red-500/10 flex-shrink-0"
                          >
                    ...
                    {editMode && (
                      <Button
                        ...
                        variant="outline"
                        className="p-3 h-auto border-dashed border-gray-500 text-gray-200 gap-2"
                      >
